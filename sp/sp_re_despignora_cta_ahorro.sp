...
-- ===============================================================
-- Procedimiento: sp_re_despignora_cta_ahorro
-- Base:         cob_ahorros
...
if exists (select 1
             from sysobjects
            where name = 'sp_re_despignora_cta_ahorro'
              and type = 'P')
   drop procedure sp_re_despignora_cta_ahorro
go

create procedure sp_re_despignora_cta_ahorro
(
...
begin
    ...
    set @w_ahora             = getdate()
    set @w_nombre_sp         = 'sp_re_despignora_cta_ahorro'
    set @w_idcta             = 0
 ...
 /*
    ...
    exec cob_remesas..sp_genera_costos
         @t_debug       = 'N'
       , @t_file        = 'NOERROR'
       , @t_from        = 'sp_re_despignora_cta_ahorro'
       , @i_fecha       = @s_date
       , @i_valor       = 1
       , @i_categoria   = 'N'
       , @i_rol_ente    = null
...
if object_id('dbo.sp_re_despignora_cta_ahorro') is not null
    print '<<< CREATED PROCEDURE dbo.sp_re_despignora_cta_ahorro >>>'
else
    print '<<< FAILED CREATING PROCEDURE dbo.sp_re_despignora_cta_ahorro >>>'
go
