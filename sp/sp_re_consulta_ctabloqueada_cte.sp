...
-- ===============================================================
-- Procedimiento: sp_re_consulta_ctabloqueada_cte
-- Base:         cob_ahorros
...
if exists (select 1
             from sysobjects
            where name = 'sp_re_consulta_ctabloqueada_cte'
              and type = 'P')
   drop procedure sp_re_consulta_ctabloqueada_cte
go

create procedure sp_re_consulta_ctabloqueada_cte
(
...
begin
    ...
    ----------------------------------------------------------------------
    set @w_resultado         = 'E'
    set @w_detalle_resultado = 'CONSULTA DE BLOQUEO DE CUENTA'
    set @w_valor_comision    = 0
    set @w_ahora             = getdate()
    set @w_nombre_sp         = 'sp_re_consulta_ctabloqueada_cte'
    set @w_idcta             = 0
    ...
    exec cob_remesas..sp_genera_costos
         @t_debug       = 'N'
       , @t_file        = 'NOERROR'
       , @t_from        = 'sp_re_consulta_ctabloqueada_cte'
       , @i_fecha       = @s_date
       , @i_valor       = 1
       , @i_categoria   = 'N'
       , @i_rol_ente    = null
...
print '@w_valor_comision  %1!',@w_valor_comision 
    ...
    if @w_return <> 0
    begin
    print 'rollback tran sp_reserva_fondos_ah'
        rollback tran
        set @w_resultado = 'F'
        select @o_cod_error = @w_return
             , @o_msg_error = mensaje
          from cobis..cl_errores
         where numero = @w_return

        if @@rowcount = 0
            set @o_msg_error =  'Error en procesar sp_reserva_fondos_ah'
...
if object_id('dbo.sp_re_consulta_ctabloqueada_cte') is not null
    print '<<< CREATED PROCEDURE dbo.sp_re_consulta_ctabloqueada_cte >>>'
else
    print '<<< FAILED CREATING PROCEDURE dbo.sp_re_consulta_ctabloqueada_cte >>>'
go
